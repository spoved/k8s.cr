...
module K8S
  ...
  class Api::Core::V1::CSIVolumeSource < ::K8S::GenericObject
    ...
    k8s_object_accessor("driver", driver : String, false, false, "Driver is the name of the CSI driver that handles this volume. Consult with your admin for the correct name as registered in the cluster.")
    k8s_object_accessor("fsType", fs_type : String, true, false, "Filesystem type to mount. Ex. \"ext4\", \"xfs\", \"ntfs\". If not provided, the empty value is passed to the associated CSI driver which will determine the default filesystem to apply.")
    k8s_object_accessor("nodePublishSecretRef", node_publish_secret_ref : ::K8S::Api::Core::V1::LocalObjectReference, true, false, "NodePublishSecretRef is a reference to the secret object containing sensitive information to pass to the CSI driver to complete the CSI NodePublishVolume and NodeUnpublishVolume calls. This field is optional, and  may be empty if no secret is required. If the secret object contains more than one secret, all secret references are passed.")
    k8s_object_accessor("readOnly", read_only : ::Bool, true, false, "Specifies a read-only configuration for the volume. Defaults to false [(read/write).]((read/write).)")
    k8s_object_accessor("volumeAttributes", volume_attributes : ::Hash(String, String), true, false, "VolumeAttributes stores driver-specific properties that are passed to the CSI driver. Consult your driver's documentation for supported values.")

    def initialize(*, driver : String? = nil, fs_type : String? = nil, node_publish_secret_ref : ::K8S::Api::Core::V1::LocalObjectReference? = nil, read_only : ::Bool? = nil, volume_attributes : ::Hash(String, String)? = nil)
      super()
      self.["driver"] = driver
      self.["fsType"] = fs_type
