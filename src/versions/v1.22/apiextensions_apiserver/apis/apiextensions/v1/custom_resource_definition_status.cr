...
module K8S
  ...
  class ApiextensionsApiserver::Apis::Apiextensions::V1::CustomResourceDefinitionStatus < ::K8S::GenericObject
    ...
    k8s_object_accessor("acceptedNames", accepted_names : ::K8S::ApiextensionsApiserver::Apis::Apiextensions::V1::CustomResourceDefinitionNames, true, false, "acceptedNames are the names that are actually being used to serve discovery. They may be different than the names in spec.")
    ...
    k8s_object_accessor("storedVersions", stored_versions : ::Array(String), true, false, "storedVersions lists all versions of CustomResources that were ever persisted. Tracking these versions allows a migration path for stored versions in etcd. The field is mutable so a migration controller can finish a migration to another version (ensuring no old objects are left in storage), and then remove the rest of the versions from this list. Versions may not be removed from `spec.versions` while they exist in this list.")
