...
::K8S::Kubernetes::Resource.define_object("StatefulSetSpec",
  ...
  properties: [
    ...
    {name: "ordinals", kind: ::K8S::Api::Apps::V1::StatefulSetOrdinals, key: "ordinals", nilable: true, read_only: false, description: "ordinals controls the numbering of replica indices in a StatefulSet. The default ordinals behavior assigns a \"0\" index to the first replica and increments the index by one for each additional replica requested. Using the ordinals field requires the StatefulSetStartOrdinal feature gate to be enabled, which is alpha."},
    {name: "persistent_volume_claim_retention_policy", kind: ::K8S::Api::Apps::V1::StatefulSetPersistentVolumeClaimRetentionPolicy, key: "persistentVolumeClaimRetentionPolicy", nilable: true, read_only: false, description: "persistentVolumeClaimRetentionPolicy describes the lifecycle of persistent volume claims created from volumeClaimTemplates. By default, all persistent volume claims are created as needed and retained until manually deleted. This policy allows the lifecycle to be altered, for example by deleting persistent volume claims when their stateful set is deleted, or when their pod is scaled down. This requires the StatefulSetAutoDeletePVC feature gate to be enabled, which is alpha.  +optional"},
    {name: "pod_management_policy", kind: String, key: "podManagementPolicy", nilable: true, read_only: false, description: "podManagementPolicy controls how pods are created during initial scale up, when replacing pods on nodes, or when scaling down. The default policy is `OrderedReady`, where pods are created in increasing order (pod-0, then pod-1, etc) and the controller will wait until each pod is ready before continuing. When scaling down, the pods are removed in the opposite order. The alternative policy is `Parallel` which will create pods in parallel to match the desired scale without waiting, and on scale down will delete all pods at once.\n\n"},
    ...
    {name: "update_strategy", kind: ::K8S::Api::Apps::V1::StatefulSetUpdateStrategy, key: "updateStrategy", nilable: true, read_only: false, description: "updateStrategy indicates the StatefulSetUpdateStrategy that will be employed to update Pods in the StatefulSet when a revision is made to Template."},
    {name: "volume_claim_templates", kind: ::Array(::K8S::Api::Core::V1::PersistentVolumeClaim), key: "volumeClaimTemplates", nilable: true, read_only: false, description: "volumeClaimTemplates is a list of claims that pods are allowed to reference. The StatefulSet controller is responsible for mapping network identities to claims in a way that maintains the identity of a pod. Every claim in this list must have at least one matching (by name) volumeMount in one container in the template. A claim in this list takes precedence over any volumes in the template, with the same name."},
